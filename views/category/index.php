...
<style>
#site-map-def .modify, #site-map .modify, #site-map-root .modify {
    width: 110px;
}

#site-map .no-children .w1 {
    padding-left: 27px;
}
</style>
...
<div id="site-map-def">
    <div class="page"><?php echo __('Category'); ?><?php if (AuthUser::hasPermission('catalog_category_edit')): ?> (<a href="#" id="toggle_reorder"><?php echo __('reorder'); ?></a>)<?php endif; ?></div>
            ...
            $(this).live('click', function() {
                if ($(this).hasClass("expanded")) {
                    ...
                    if ($('#page_'+parentId).children('ul').length == 0) {
                        $('#busy-'+parentId).show();
                        $.get("<?php echo get_url('plugin/catalog/categoryChildren/'); ?>"+parentId+'/'+'1', function(data) {                        
                ...
                (function persistExpanded() {
                    ...
                    document.cookie = 'catalog_category_expanded_rows=' + rows + ';'
                })();
            });
        };
            ...
            $('ul#site-map').nestedSortable({
                ...
                stop: function(event, ui) {                    
                    var order = $("ul#site-map").nestedSortable('serialize');
                     ...
                     $.ajax({
                        type: 'post',
                        url: '<?php echo get_url('plugin/catalog/reorder/category'); ?>',
                        data: order,
                        cache: false
                    });  
