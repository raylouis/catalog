...
<?php foreach($children as $child): ?> 
    <li id="page_<?php echo $child->id; ?>" class="node level-<?php echo $level; if ( ! $child->has_children) echo ' no-children'; else if ($child->is_expanded) echo ' children-visible'; else echo ' children-hidden'; ?>">
      ...
      <div class="page">
        ...
        <div class="view-page">
            <a href="<?php echo $child->url(); ?>" target="_blank"><img src="<?php echo URI_PUBLIC;?>wolf/icons/open-16.png" align="middle" alt="<?php echo __('View category'); ?>" title="<?php echo __('View category'); ?>" /></a>
        </div>
      <div class="modify">
        <?php if (AuthUser::hasPermission('catalog_category_add')): ?>
            <a class="add-child-link" href="<?php echo get_url('plugin/catalog/category/add', $child->id); ?>"><img src="<?php echo URI_PUBLIC;?>wolf/icons/add-16.png" align="middle" title="<?php echo __('Add child'); ?>" alt="<?php echo __('Add child'); ?>" /></a>&nbsp;
        <?php endif; ?>
        
        <?php if (AuthUser::hasPermission('catalog_category_delete')): ?>
            <a class="remove" href="<?php echo get_url('plugin/catalog/category/delete', $child->id); ?>" onclick="return confirm('<?php echo __('Are you sure you wish to delete :categorytitle and its underlying categories?', array(':categorytitle' => $child->title)); ?>');"><img src="<?php echo URI_PUBLIC;?>wolf/icons/delete-16.png" alt="<?php echo __('Delete'); ?>" title="<?php echo __('Delete'); ?>" /></a>
        <?php endif; ?>
      </div>
      </span>
